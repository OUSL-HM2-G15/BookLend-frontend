...
const BookDetails = () => {
  ...
  const hasActiveRequest = location.state?.hasActiveRequest || false;
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        const url = showOwnerInfo
          ? `${process.env.REACT_APP_API_URL}/borrow-requests/${id}/book-details`
        ...
        <div className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div >
            ...
            >
              <span>
                <button
                  disabled={book.status !== "Available" || hasActiveRequest}
                  className={`py-2 px-4 rounded-lg mt-6 transition 
    ${book.status !== "Available" || hasActiveRequest
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-gray-800">
              ...
              <p>
                {showOwnerInfo ? book.owner?.phone :
                  <span className="text-gray-400">Hidden</span>
                }
              </p>
              ...
              <p>
                {showOwnerInfo ? book.owner?.whatsappNumber :
                  <span className="text-gray-400">Hidden</span>
                }
              </p>
            </div>
